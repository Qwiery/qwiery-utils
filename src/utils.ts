...
export const Utils = {
	 ...
	 * @return {string}
	 */
	amountToMoneyFormat(amount: any, currency: string = "USD"): string {
	 ...
	 * @returns {string}
	 */
	camelTypeToTitle(name: string): string {
	 ...
	 * @returns {string}
	 */
	createTempFile(fileName: string = null, extension: string = "tmp"): string {
	 ...
	 */
	deepReplace(rootObject: object, substitute: object, path: string) {
		if (path === undefined) {
			path = "/";
		}
		if (path === "." || path === "/") {
	 ...
	 */
	deleteFileOrDirectory(filePath: string) {
	 ...
	 */
	getArguments(options: any[]): [number, any[]] {
		if (_.isNil(options)) {
			return [0, []];
		}
		return [
	 ...
	 * @returns {*|null}
	 */
	getDataFromSpecs(data: any | function, id = null, labels = null): any | null {
		if (_.isNil(data)) {
			return null;
		}
	 ...
	 * @returns {string|null}
	 */
	getIdFromSpecs(data: any | function = null, id = null, labels = null): string | null {
	 ...
	 * @returns {*|null}
	 */
	getLabelsFromSpecs(data: any | function = null, id = null, labels = null): any | null {
		if (!_.isNil(labels)) {
			if (_.isArray(labels)) {
				return _.uniq(
					labels
	 ...
	 */
	getNodeSpecs(data = null, id = null, labels = null): { id: string; data: any; labels: string[]; } | null {
	 ...
	 * @returns {{}|*}
	 */
	getReducedPlainObject(obj: any, exclude: string[] | string = []): {} | any {
		if (_.isNil(obj)) {
			return {};
		}
	 ...
	 * getSubFolders(["/a/b"])
	 */
	getSubFolders(paths: string[], current = "/"): any[] | any {
	 ...
	 * @returns {string}
	 */
	getTempFilePath(fileName: string = null, extension: string = "tmp"): string {
	 ...
	 * @returns string[]
	 */
	getTripleSpecs(s: string | string[], p: string, o: string) {
		if (_.isNil(s)) {
	 ...
	 * @returns {*[]|*}
	 */
	histogram(data, bins = 10): any[] | any {
		if (bins === 1) {
			return [data.length];
		}
	 ...
	 * @returns {*}
	 */
	insertAt(coll, item, index): any {
		coll.splice(index, 0, item);
		return coll;
	},

	/**
	 * Returns whether the given name is alpha-numeric.
	 ...
	 * @returns {boolean}
	 */
	isDirectory(somePath: string): boolean {
	 ...
	 * @returns {boolean}
	 */
	isSimpleString(s: string): boolean {
		if (Utils.isEmpty(s)) {
			return false;
		}
		if (!_.isString(s)) {
	 ...
	 * @returns {any[]}
	 */
	mergeArrays(ar1: any[], ar2: any[]): any[] {
		if (_.isNil(ar1)) {
	 ...
	 */
	mustache(template: string, data: any) {
	 ...
	 * @returns {boolean}
	 */
	positiveNumberBetween(i: number, a, b, inclusiveEnd = true, allowZero = false): boolean {
		if (!_.isNumber(i)) {
	...
	randomId(length: number = 10, onlyLetters: boolean = false): string {
	 ...
	 * @returns {number}
	 */
	randomInteger(from: number = 0, to: number = 1e10): number {
		if (to - from < 1) {
			throw new Error(
	 ...
	 */
	randomLetter(mixedCase = true): string {
	 ...
	 * @return {any}
	 */
	randomLetters(length = 10, mixedCase = true): any {
	 ...
	 */
	validateJsonIsForType(json: any, typeName: string) {
