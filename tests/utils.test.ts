...
describe("Utils", function () {
    ...
    it('should validate Qwiery nodes', () => {
        expect(Utils.isQwieryNode({})).toBeFalsy();
        expect(Utils.isQwieryNode({a: 4})).toBeFalsy();
        expect(Utils.isQwieryNode({edge: []})).toBeFalsy();
        expect(Utils.isQwieryNode([1, 2, 3])).toBeFalsy();
        expect(Utils.isQwieryNode(new Error("a"))).toBeFalsy();
        expect(Utils.isQwieryNode({id: {k: 4}})).toBeFalsy();

        expect(Utils.isQwieryNode({id: "a"})).toBeTruthy();
    });
    it('should validate Qwiery edges', () => {
            expect(Utils.isQwieryEdge({})).toBeFalsy();
            expect(Utils.isQwieryEdge({a: 4})).toBeFalsy();
            expect(Utils.isQwieryEdge({a: []})).toBeFalsy();
            expect(Utils.isQwieryEdge([1, 2, 3])).toBeFalsy();
            expect(Utils.isQwieryEdge(new Error("a"))).toBeFalsy();
            expect(Utils.isQwieryEdge({id: "a"})).toBeFalsy();
            expect(Utils.isQwieryEdge({sourceId: Error, targetId: "c"})).toBeFalsy();

            expect(Utils.isQwieryEdge({sourceId: "b", targetId: "c"})).toBeTruthy();
            expect(Utils.isQwieryEdge({sourceId: 3.5, targetId: "c"})).toBeTruthy();
            expect(Utils.isQwieryEdge({id: "a", sourceId: "b", targetId: "c"})).toBeTruthy();
        }
    )

    it('should validate Qwiery graphs', () => {
        expect(Utils.isGraphLike({})).toBeFalsy();
        expect(Utils.isGraphLike({a: 4})).toBeFalsy();
        expect(Utils.isGraphLike({a: []})).toBeFalsy();
        expect(Utils.isGraphLike([1, 2, 3])).toBeFalsy();
        expect(Utils.isGraphLike(new Error("a"))).toBeFalsy();
        expect(Utils.isGraphLike({id: "a"})).toBeFalsy();
        expect(Utils.isGraphLike({sourceId: Error, targetId: "c"})).toBeFalsy();
        expect(Utils.isGraphLike({sourceId: "b", targetId: "c"})).toBeFalsy();
        expect(Utils.isGraphLike({sourceId: 3.5, targetId: "c"})).toBeFalsy();
        expect(Utils.isGraphLike({id: "a", sourceId: "b", targetId: "c"})).toBeFalsy();
        expect(Utils.isGraphLike({nodes: [3, 4]})).toBeFalsy();
        expect(Utils.isGraphLike({nodes: String()})).toBeFalsy();

        expect(Utils.isGraphLike({nodes: [], edges: []})).toBeTruthy();
        expect(Utils.isGraphLike({nodes: [], edges: [], id: "a"})).toBeTruthy();
        expect(Utils.isGraphLike({nodes: [], edges: [], id: "a", description: "b"})).toBeTruthy();
        expect(Utils.isGraphLike({nodes: [], edges: [], id: "a", description: "b", name: "c"})).toBeTruthy();
    })
});
